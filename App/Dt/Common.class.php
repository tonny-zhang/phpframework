 ...
 *
 */
class Dt_Common extends Dt_Base {
	protected $db = null;
	protected $dbTable = '';
	protected $useMcInfo = false;
	protected $useMcList = false;
	protected $useMcTotal = false;
	protected $pageSize = 100; //缓存中记录的个数
	public function __construct($db = 'default', $dbTable = '', $useMcInfo = false, $useMcList = false, $useMcTotal = false) {
		$this->dbTable = $dbTable;
		$this->useMcInfo = $useMcInfo;
		$this->useMcList = $useMcList;
		$this->useMcTotal = $useMcTotal;

		//缓存定义
		$this->mcConfig = array(
			'common_list_total' => array('key'=> CACHE_KEY_COMMON."_{$db}.{$dbTable}_1_%s", 'lifetime'=> 86400, 'mc' => 'common'),//1天
			'common_list' => array('key'=> CACHE_KEY_COMMON."_{$db}.{$dbTable}_2_%s", 'lifetime'=> 86400, 'mc' => 'common'),//1天
			'common_info' => array('key'=> CACHE_KEY_COMMON."_{$db}.{$dbTable}_3_%s", 'lifetime'=> 86400, 'mc' => 'common'),//1天
		);
