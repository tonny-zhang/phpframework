...
class Helper_Uploader_File
{
    ...
    function isValid($allowed_types = null, $max_size = null)
    {
        ...
        if ($allowed_types)
        {
            $allowed_types = Core::normalize($allowed_types);
            ...
            $filename = strtolower($this->getfname($this->filename()));
            $extnames = Core::normalize($filename, '.');
    ...
    /**
     * function getfname
     * 参数：$url 要获取文件名及文件后辍的路径
     * 如何找到返回$filename,未找到返回false
     */
    // 流程控制语句
